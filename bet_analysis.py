...
Reads instrument XLS output and computes:
  - Isotherm type classification  (IUPAC Type I–VI)
...
def read_bet_xls(filepath: str) -> dict:
    ...
    df = raw["AdsDes"]
    # find row where 'ADS' label sits
    ads_start = df[df.iloc[:, 0] == "ADS"].index[0] + 1
    des_start = df[df.iloc[:, 0] == "DES"].index[0] + 1

    def _extract(start, end_label):
        rows = []
        for i in range(start, len(df)):
            if df.iloc[i, 0] == end_label:
                break
            row = df.iloc[i]
            try:
                pp0 = float(row[5])
    ...
    # ── BET plot points ───────────────────────────────────────
    df_b = raw["BET"]
    no_row = df_b[df_b.iloc[:, 0] == "No"].index[0] + 1
    bet_pts = []
    ...
    # starting/end point used by instrument
    start_pt = int(df_b[df_b.iloc[:, 0] == "Starting point"].iloc[0, 3])
    end_pt   = int(df_b[df_b.iloc[:, 0] == "End point"].iloc[0, 3])

    # ── BJH pore size distribution ────────────────────────────
    df_j = raw["BJH"]
    no_row_j = df_j[df_j.iloc[:, 0] == "No"].index[0] + 1
    bjh_rows = []
...
def classify_isotherm(ads: np.ndarray, des: np.ndarray) -> dict:
    ...
    if low_mask.sum() > 2:
        ...
        concave_low = float(d2.mean()) < 0   # concave-up = negative d2
    else:
        concave_low = True
    ...
    if is_stepped and not has_hyst:
        ...
        if steep_init and has_plateau:
            iso_type = "Type I"
            explanation = ("Steep rise at low p/p₀ followed by plateau. "
    ...
    # Note: the BET C constant further supports classification:
    # C >> 50 → strong interaction (Type I/II/IV), C < 2 → weak (Type III/V)
...
def classify_hysteresis(ads: np.ndarray, des: np.ndarray) -> dict:
    ...
    # Sort desorption ascending so interp works
    ...
    hyst_area = float(np.trapezoid(hyst, p_grid))
    norm_area = hyst_area / (Va_a.max() + 1e-9)     # 0 = narrow, 1 = wide

    # ── Feature 2: slope ratio (des steeper than ads?) ─────────
    ads_slopes  = np.abs(np.gradient(Va_a_g, p_grid))
    des_slopes  = np.abs(np.gradient(Va_d_g, p_grid))
    # restrict to mid-range 0.3–0.95 to avoid edge artefacts
    ...
    # ── Feature 3: loop shape — triangular vs parallelogram ────
    # peak of hysteresis relative to centre of p_grid
    peak_idx   = np.argmax(hyst)
    peak_pos   = p_grid[peak_idx]          # <0.5 → left-skewed (H2), >0.5 → H3
    ...
    # ── Feature 5: flatness at low p/p0 (H4 signature) ─────────
    ...
    if low_ads_mask.sum() > 3:
        ...
        is_flat_low = flat_slope < 30   # small rise per unit p/p0
    else:
        is_flat_low = False

    # ── Feature 6: closure point ───────────────────────────────
    # For N2 at 77K, forced closure at p/p0 ~ 0.42
    # Below that, instrument-forced → doesn't distinguish type
    ...
    # H1: both branches steep, narrow loop, uniform cylinder
    if ratio_mean < 2.0 and norm_area < 0.15 and has_plateau:
        scores["H1"] += 3
    if ratio_max < 2.5:
        scores["H1"] += 1

    # H2: triangular, steep desorption, ink-bottle
    if ratio_max > 2.0:
        scores["H2"] += 3
    if is_left_skewed:
        scores["H2"] += 2
    ...
    # H3: no plateau, continues to rise near p/p0=1, slit pores
    if not has_plateau:
        scores["H3"] += 3
    if norm_area > 0.20:
        scores["H3"] += 2
    ...
    # H4: flat, narrow loop, micropore + slit
...
def verify_bet(bet_pts: np.ndarray, summary: dict) -> dict:
    ...
    BET linear form: 1/[Va(p0/p-1)] = (C-1)/(Vm·C) · (p/p0) + 1/(Vm·C)
    → Vm = 1/(slope + intercept),  C = 1 + slope/intercept
    → S_BET = Vm × 4.353 m²/g  (for N₂, σ=0.162 nm², standard formula)
    """
    # start_pt and end_pt are the "No." labels in the BET sheet
    # which match directly to array indices (No=0 is origin at index 0)
    s   = summary["start_pt"]        # e.g. 4 → index 4
    e   = summary["end_pt"] + 1      # e.g. 9 → slice up to index 10
    pts = bet_pts[s:e]
    ...
    S_BET = Vm * 4.353   # N2 cross-section 0.162 nm² × NA × 1e-20 → scaled to m²/g
...
def plot_all(data: dict, iso_cls: dict, hyst_cls: dict,
    ...
    4-panel publication figure:
      ...
      [B] BET Plot
      [C] BJH Differential Pore Size Distribution
    ...
    # ── [C] BJH Differential PSD ─────────────────────────
    # NOTE (Microtrac/IUPAC): BJH from adsorption branch is preferred.
    # Desorption BJH shows a spurious ~3.4 nm peak due to N2 cavitation
    # at 77K (p/p0 ≈ 0.42) — not a real pore. Adsorption branch avoids this.────
    ax = axes[2]
    rp   = bjh[:, 0] * 2   # radius → diameter (nm)
    dVdr = bjh[:, 1]
    ...
    ax.xaxis.set_minor_locator(AutoMinorLocator())
    ax.yaxis.set_minor_locator(AutoMinorLocator())
    # cavitation artifact warning: N2 @77K forced closure ~3.4 nm (des. branch)
    ax.axvline(3.4, ls=":", lw=0.8, color="0.6", alpha=0.7)
    ax.text(3.6, ax.get_ylim()[1]*0.01 if ax.get_ylim()[1]>0 else 0.001,
    ...
    # BET vs BJH horizontal reference lines
...
def print_report(data: dict, iso_cls: dict, hyst_cls: dict,
    ...
    rows = [
        ...
        ["BJH Peak Radius (2×)", f"{s['rp_peak_BJH']*2:.2f}", "nm"],
    ]
